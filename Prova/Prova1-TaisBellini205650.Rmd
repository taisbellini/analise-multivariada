...
gg = ggpairs(x_f[,-3], mapping=aes(colour=as.factor(x_f[,3])), upper=list(combo= 'blank', continuous='blank'), title = "Gráfico 1") 
...
Para isto, vamos calcular a distância entre as notas antes e depois do reforço para cada matéria e aplicar o teste com a estatística $T^2$:
```{r}
d_barra = colMeans(d)
S = cov(d)
mu=c(0,0,0)
...
Observa-se que, em ambos os intervalos, apenas a matéria de Química não engloba o zero, ou seja, o reforço só foi influente nesta matéria. Isto também pode ser observado no Gráfico 1, onde observamos que a média dos dados da matéria de Química se moveu mais significativamente em relação as outras.  
